...
- `2026-03-11`：更新 `minimalAuthContracts` 的最小 Header 约束：`createQRLogin` / `queryQRLoginStatus` 增加 `x-rpc-device_fp`，`avatar_basic_list` / `batch_avatar_detail_v2` 增加 `x-rpc-device_id`，`note` 增加 `x-rpc-device_fp`。
